...
export const ChapterDetail: React.FC = () => {
    ...
    const themes = {
        ...
        dark: { bg: 'bg-[#0a0a0c]', text: 'text-slate-300', nav: 'bg-[#1a1b26]/80', border: 'border-slate-800/50' },
        sepia: { bg: 'bg-[#f4ecd8]', text: 'text-[#433422]', nav: 'bg-[#f4ecd8]/90', border: 'border-[#d3c4a3]/60' },
        twilight: { bg: 'bg-[#0f172a]', text: 'text-slate-300', nav: 'bg-[#0f172a]/80', border: 'border-slate-700/50' }
    };
    ...
    const handleSaveEdit = async (e: React.FormEvent) => {
        ...
        setUpdating(true);
        try {
            await updateChapter(chapterId, editTitle, editContent);
            setChapter({ ...chapter, title: editTitle, content: editContent });
            setIsEditOpen(false);
    ...
    return (
        <div className={`${currentTheme.bg} min-h-screen ${currentTheme.text} transition-colors duration-300`}>
            ...
            <div className="max-w-4xl mx-auto px-4 py-10">
                ...
                <div 
                    className={`prose max-w-none leading-relaxed text-justify [&_p]:mb-6 [&_p]:min-h-[1rem] ${readSettings.fontFamily === 'serif' ? 'font-serif' : 'font-sans'}`}
                    style={{ 
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <button 
                                        ...
                                        className={`py-2.5 flex flex-col items-center justify-center rounded-2xl border-2 transition-all bg-[#0a0a0c] text-slate-300 shadow-sm ${readSettings.theme === 'dark' ? 'border-primary shadow-primary/20 scale-105' : 'border-transparent hover:border-slate-800'}`}
                                    >
                                        <Moon className="w-4 h-4 mb-1"/> Đen
                                    </button>
                                    <button 
                                        onClick={() => setReadSettings({...readSettings, theme: 'twilight'})}
                                        className={`py-2.5 flex flex-col items-center justify-center rounded-2xl border-2 transition-all bg-[#0f172a] text-blue-100 shadow-sm ${readSettings.theme === 'twilight' ? 'border-primary shadow-primary/20 scale-105' : 'border-transparent hover:border-slate-700'}`}
                                    >
